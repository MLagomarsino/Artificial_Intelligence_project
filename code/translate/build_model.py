...
def variables_to_numbers(effect, conditions):
    ...
    for i, arg in enumerate(effect.args):
        if isinstance(arg, pddl.Variable):
            rename_map[arg] = i
            new_effect_args[i] = i
...
class JoinRule(BuildRule):
    ...
    def validate(self):
        ...
        left_vars = set([v for v in left_args if isinstance(v, int) or isinstance(v, pddl.Variable)])
        right_vars = set([v for v in right_args if isinstance(v, int) or isinstance(v, pddl.Variable)])
        eff_vars = set([v for v in eff_args if isinstance(v, int) or isinstance(v, pddl.Variable)])
...
class ProductRule(BuildRule):
    ...
    def validate(self):
        assert len(self.conditions) >= 2, self
        cond_vars = [set([v for v in cond.args if isinstance(v, int) or isinstance(v, pddl.Variable)])
                     for cond in self.conditions]
        all_cond_vars = reduce(set.union, cond_vars)
        eff_vars = set([v for v in self.effect.args
                        if isinstance(v, int) or isinstance(v, pddl.Variable)])
