...
# for example if there is a derived predicate in the input that only depends on
...
def translate_strips_conditions_aux(conditions, dictionary, ranges, comparison_axioms,
    ...
    for fact in conditions:
        if isinstance(fact, pddl.FunctionComparison) or isinstance(fact, pddl.NegatedFunctionComparison):
...
def unsolvable_sas_task(msg):
    ...
    num_axioms = []
    comp_axioms = []
    return sas_tasks.SASTask(variables, init, goal, operators, temp_operators, axioms, num_axioms, comp_axioms)
