...
# ADDED for CDCL: keep track of variable's activity in conflicts
class Conflict:
...
class Formula:
    ...
    # original_cl    ADDED: the index below which a clause is not learnt
    # clause_list    the list of Clause objects
    # variable_list  the list of Variable objects
    # conflict list  ADDED: number of occurrences in conflict clauses  
    ...
    def __init__(self, inputFile):
        ...
        self.original_cl = len(self.clause_list) # ADDED
        return
    ...
    def read_cnf(self,fileName):
        ...
        for line in inputFile:
            if (line[0] == 'c'):
                ...
                self.conflict_list = [Conflict() for x in range(self.open_var + 1)]
            else:
                cleanLine = line.rstrip(' \n')
                # print('=' + cleanLine + '=')
                # MODIFIED: more robust
                lit_list = [int(x.strip(' ')) for x in cleanLine.split()][:-1]
                #print([(x.strip(' ')) for x in cleanLine.split()][:-1])
                self.add_clause(lit_list)
                
        inputFile.close()
        return

    # ADDED: works for initial and learnt clauses
    def add_clause(self, lit_list):
        ...
        for l in lit_list:
            ...
            if (l > 0):
                ...
                # If the clause is added during execution, update logs
                # It is assumed that the clause cannot be subsumed
                if (v.value == -1):
                    clause.open -= 1
            else:
    ...
    def do_subsume(self, v_index, cl_index_list):
        for cl_index in cl_index_list:
            # ADDED: Do not subsume learned clauses
    ...
    def undo_subsume(self, v_index, cl_index_list):
        for cl_index in cl_index_list:
            # ADDED: Do not unsubsume learned clauses
            if cl_index < self.original_cl:
                cl = self.clause_list[cl_index]
