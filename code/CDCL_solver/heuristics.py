...
class VsidsHeuristic:
    ...
    def run(self, formula, stack):
        ...
        if (self.invocations == 1):
            ...
            # MARTA: random choise of variable
        ...
        if (self.invocations % self.period == 0):
            ...
            self.priority = \
                sorted(range(len(formula.variable_list)),\
                       key = lambda x : formula.conflict_list[x].pos +\
                                        formula.conflict_list[x].neg)
        # Choose variable with maximum priority
        # MARTA: scan the list of priorities on the reverse order (we want to find the highest)
